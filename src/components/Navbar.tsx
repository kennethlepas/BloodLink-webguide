...
export default function Navbar() {
    ...
    return (
        <>
            ...
            {/* Mobile menu backdrop - rendered outside nav to avoid stacking context issues */}
            <div
                className={`fixed inset-0 z-[60] md:hidden transition-all duration-300 ${isOpen ? "opacity-100 pointer-events-auto" : "opacity-0 pointer-events-none"}`}
                onClick={() => setIsOpen(false)}
            >
                <div className="absolute inset-0 bg-slate-900/40 backdrop-blur-sm" />
            </div>

            {/* Mobile menu drawer - rendered outside nav with higher z-index */}
            <div className={`fixed top-0 right-0 bottom-0 w-4/5 max-w-sm z-[70] bg-white shadow-2xl md:hidden transition-transform duration-500 ease-out transform ${isOpen ? "translate-x-0" : "translate-x-full"}`}>
